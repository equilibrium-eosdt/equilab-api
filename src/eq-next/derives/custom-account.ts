import type { Asset, SignedBalance } from "@equilab/api/eq-next/interfaces";
...
import { u64FromCurrency } from "@equilab/api/eq-next/util";
...
const transformBalanceStorage = (
  ...
  const boundCreator = (account: AccountIndex | AccountId | Address | string) =>
    query.creator([account, { 0: u64FromCurrency(arg) }]);
...
export const createCustomAccount = (cur: string) => (
  ...
  return transformBalanceStorage(
    api.query.eqBalances.account as EqBalanceDoubleMap,
    cur,
    transform,
  );
};
